...
function findRepo(start, gitBinary = "git") {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        ...
        const status = yield subprocess_1.getSubprocessOutput(gitBinary, ["rev-parse", "--git-dir"], { cwd: start });
...
class ObjReader {
    ...
    getObj() {
        return {
            ...
            data: (this.objType === rawtypes_1.ObjType.COMMIT || this.objType === rawtypes_1.ObjType.ATAG) ? this.mergeBuffer() : null,
        };
    }
    /**
    ...
    readMetaData(firstChunk) {
        ...
        if (!metadataLine) {
            throw new Error(`metadata not found: ${JSON.stringify(firstChunk)} / ${JSON.stringify(util_1.chunkToLines(firstChunk))}`);
        }
    ...
    resolveRef(ref) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            if (parser.isRefSymDest(ref.dest)) {
                ...
                return resolved.concat(yield this.resolveRef(next));
            }
            else if (parser.isSHA1(ref.dest)) {
                const destObj = yield this.readObject(ref.dest);
                if (rawtypes_1.DetectObjType.isAnnotatedTag(destObj)) {
                    // reconstruct a GitRef from object
                    const destObjAsRef = {
                        type: rawtypes_1.RefType.ATAG,
                        path: `refs/tags/${destObj.name}`,
                        dest: destObj.dest
                    };
                    return [destObj].concat(yield this.resolveRef(destObjAsRef));
                }
                else {
                    // FIX resolve further when the obj is atag
