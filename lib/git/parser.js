...
exports.ObjTypeMappings = util_1.freeze({
    commit: rawtypes_1.ObjType.COMMIT,
    tag: rawtypes_1.ObjType.ATAG,
    blob: rawtypes_1.ObjType.BLOB,
    tree: rawtypes_1.ObjType.TREE,
});
...
function parseBranch(line, path) {
    if (line.match(exports.PATTERNS.commit_sha1))
        return {
            type: rawtypes_1.RefType.BRANCH,
            path: path,
            dest: line
        };
...
function parseTag(line, path) {
    if (line.match(exports.PATTERNS.commit_sha1))
        return {
            type: rawtypes_1.RefType.UNKNOWN_TAG,
            path: path,
            dest: line
        };
...
exports.isRefSymDest = isRefSymDest;
/**
...
function parseRefList(ref_lines) {
    ...
    for (const line of ref_lines.filter(util_1.isTruthy)) {
        ...
        if (dest_type === "commit"
            ...
            refs.push({ name: refname, type: rawtypes_1.RefType.HEAD, dest: dest_sha1 });
            continue;
        }
        else if (dest_type === "commit"
            && exports.PATTERNS.refpath.local_branch.exec(refname)) {
            refs.push({ name: refname, type: rawtypes_1.RefType.BRANCH, dest: dest_sha1 });
            continue;
        }
        else if (dest_type === "commit"
            ...
            refs.push({ name: refname, type: rawtypes_1.RefType.TAG, dest: dest_sha1 });
            continue;
        }
    ...
    for (const l of lines) {
        ...
        if (matched) {
            ...
            if (exports.PATTERNS.refpath.tag.exec(refpath)) {
                found.push({ dest: sha1, type: rawtypes_1.RefType.UNKNOWN_TAG, path: refpath });
            }
            else if (exports.PATTERNS.refpath.local_branch.exec(refpath)) {
                found.push({ dest: sha1, type: rawtypes_1.RefType.BRANCH, path: refpath });
            }
...
function parseCommit(sha1, lines) {
    ...
    lines = lines.slice();
    const result = {
        type: rawtypes_1.ObjType.COMMIT,
        sha1: sha1,
        author: null,
        author_at: null,
        committer: null,
