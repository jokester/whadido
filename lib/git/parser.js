...
exports.ObjTypeMappings = type_1.freeze({
    commit: 1 /* COMMIT */,
    tag: 2 /* ATAG */,
    blob: 4 /* BLOB */,
    tree: 3 /* TREE */,
});
exports.PATTERNS = type_1.freeze({
    refpath: type_1.freeze({
        local_head: /^HEAD$/,
...
function parseTag(line, path) {
    if (line.match(exports.PATTERNS.commit_sha1))
        return {
            type: types_1.Ref.Type.UNKNOWN_TAG,
            path: path,
            dest: line
        };
...
function extractLocalBranch(refpath) {
    const matched = exports.PATTERNS.refpath.local_branch.exec(refpath);
    return matched && matched[1];
}
exports.extractLocalBranch = extractLocalBranch;
/**
 *
 *
 * @export
    ...
    for (const l of lines) {
        ...
        if (matched) {
            ...
            if (exports.PATTERNS.refpath.tag.exec(refpath)) {
                found.push({ dest: sha1, type: types_1.Ref.Type.UNKNOWN_TAG, path: refpath });
            }
            else if (exports.PATTERNS.refpath.local_branch.exec(refpath)) {
                found.push({ dest: sha1, type: types_1.Ref.Type.BRANCH, path: refpath });
            }
