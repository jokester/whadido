...
let TestSubProcess = class TestSubProcess {
    true_returns_0() {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            const result = yield subprocess_1.getSubprocessOutput('true');
            chai_1.expect(result.stderr).deep.eq([""]);
        });
    }
    false_returns_1() {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                chai_1.expect(e).not.eq(err);
            }
        });
    }
    captures_stdout1() {
        return subprocess_1.getSubprocessOutput('/bin/echo', ['aa', 'bb'])
            .then(result => {
            chai_1.expect(result.stdout).to.deep.eq(['aa bb', '']);
        });
    }
    captures_stdout2() {
        return subprocess_1.getSubprocessOutput('/bin/echo', ["-n", 'aa', 'bb'])
            .then(result => {
            chai_1.expect(result.stdout).to.deep.eq(['aa bb']);
        });
    }
...
let TestGitRepo = class TestGitRepo {
    ...
    openTestRepo2() {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            const path = path_1.join(__dirname, '..', '..', 'test', 'TypeScript.git', 'hooks');
            const foundRepo = yield repo.findRepo(path);
            return repo.openRepo(foundRepo);
        });
    }
    ...
    openDevRepo() {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            const path = path_1.join(__dirname, '..', '..', 'test');
            const foundRepo = yield repo.findRepo(path);
            return repo.openRepo(foundRepo);
        });
    }
    ...
    findRefFiles() {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            const refsRoot = path_1.join(__dirname, '..', '..', 'test', 'node-libtidy.git', 'refs');
            const found = yield repo.listRefFiles(refsRoot);
            chai_1.expect(found).deep.eq([
                path_1.join(refsRoot, 'tags', 't2'),
                path_1.join(refsRoot, 'remotes', 'origin', 'HEAD'),
            ]);
        });
    }
    readNonpackedRef() {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            const testRepo = yield this.openTestRepo();
            const nonpacked = yield testRepo.readNonpackedRef();
            chai_1.expect(nonpacked).deep.eq([
                {
    ...
    readNonpackedRef$$() {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            const devRepo = yield this.openDevRepo();
            const nonpacked = yield devRepo.readNonpackedRef();
            chai_1.expect(nonpacked).deep.eq([]);
        });
    }
    readObjRaw1() {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            const testRepo = yield this.openTestRepo();
            try {
                yield testRepo.readObjRaw("NOT");
                throw "should not be here";
            }
            catch (e) {
                chai_1.expect(e).instanceOf(Error);
                chai_1.expect(e.message).eq("object \"NOT\" is missing");
            }
        });
    }
    readObjRaw2() {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            const testRepo = yield this.openTestRepo();
            const rawObj = yield testRepo.readObjRaw("HEAD");
            const lines = util.chunkToLines(rawObj.data);
            // metadata line: "414c5870b27970db0fa7762148adb89eb07f1fe0 commit 347"
            chai_1.expect(rawObj.type).eq(rawtypes.ObjType.COMMIT);
            chai_1.expect(rawObj.data.length).eq(347);
            chai_1.expect(rawObj.sha1).eq("414c5870b27970db0fa7762148adb89eb07f1fe0");
            chai_1.expect(lines.length).eq(9);
            chai_1.expect(lines[3]).eq("committer Martin von Gagern <Martin.vGagern@gmx.net> 1478766514 +0100");
            chai_1.expect(lines[7]).eq("This line got into master by accident, as gulp support isn't ready yet.");
        });
    }
    readCommitObj() {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            const testRepo = yield this.openTestRepo();
            const obj = yield testRepo.readObject("931bbc96");
            chai_1.expect(obj.sha1).eq("931bbc96dc534e907479c0b82f0bf48598ad6b7c");
            if (rawtypes.DetectObjType.isCommit(obj)) {
                chai_1.expect(obj.author).deep.eq({ name: "Martin von Gagern", email: "Martin.vGagern@gmx.net" });
                chai_1.expect(obj.parent_sha1).deep.eq(["0b017e41fc65a3c3193fd410d6d35274d7bb7f71"]);
                chai_1.expect(obj.commit_at).deep.eq({ tz: "+0200", "utc_sec": 1463522581 });
            }
            else {
                throw "not a commit";
            }
    ...
    readAtagObj() {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            const testRepo = yield this.openTestRepo();
            const obj = yield testRepo.readObject("07fc");
            chai_1.expect(obj.sha1).eq("07fc5636f0359a857a5f9ecd583fcd56d6edb83b");
            chai_1.expect(obj.type).eq(rawtypes.ObjType.ATAG);
            if (rawtypes.DetectObjType.isAnnotatedTag(obj)) {
                chai_1.expect(obj.destType).eq(rawtypes.ObjType.COMMIT);
                chai_1.expect(obj.dest).eq("414c5870b27970db0fa7762148adb89eb07f1fe0");
                chai_1.expect(obj.tagger).deep.eq({ name: "Martin von Gagern", email: "Martin.vGagern@gmx.net" });
                chai_1.expect(obj.tagged_at).deep.eq({ tz: "+0100", "utc_sec": 1478766625 });
                chai_1.expect(obj.name).eq("v0.3.2");
                chai_1.expect(obj.message.length).eq(10);
                chai_1.expect(obj.message[0]).eq("Version 0.3.2");
                chai_1.expect(obj.message[2]).eq("* Fixed gulp import introduced in 0.3.1 by accident");
                chai_1.expect(obj.message[6]).eq("dFuDzQCeKXpBXRfcDBf9nKhPcVBoMoZgx1oAnRoeTCxkIsPhwSqiYgh3PpRYLpn1");
                chai_1.expect(obj.message[8]).eq("-----END PGP SIGNATURE-----");
                chai_1.expect(obj.message[9]).eq("");
            }
            else {
    ...
    readAllObjects1() {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            const objectListFile = path_1.join(testRepo.repoRoot, "objects-list.txt");
            const objectList = yield util.readLines(objectListFile);
            for (const l of objectList) {
                const sha1 = l.slice(0, 40);
                if (sha1) {
                    const obj = yield testRepo.readObject(sha1);
                }
            }
        });
    }
    readAllObjects2() {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            const testRepo = yield this.openTestRepo2();
            // a list of all objects, generated with `git cat-file --batch-all-objects --batch-check`
            const objectListFile = path_1.join(testRepo.repoRoot, "objects-list.txt");
            const objectList = yield util.readLines(objectListFile);
    ...
    resolveRef() {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            const repo = yield this.openTestRepo();
            const refs = yield repo.listRefs();
            const localHead = refs[0];
            chai_1.expect(localHead).deep.eq({ dest: "refs/heads/master", path: "HEAD", type: rawtypes.RefType.HEAD });
            const resolved = yield repo.resolveRef(localHead);
            chai_1.expect(resolved[0].type).eq(rawtypes.RefType.HEAD);
            chai_1.expect(resolved[1].type).eq(rawtypes.RefType.BRANCH);
            chai_1.expect(resolved[2].type).eq(rawtypes.ObjType.COMMIT);
            const nestedAtag = refs[9];
            // expect(refs).deep.eq([]);
            chai_1.expect(nestedAtag.path).eq('refs/tags/t2');
            const resolved2 = yield repo.resolveRef(nestedAtag);
            chai_1.expect(resolved2).deep.eq([]);
        });
    }
};
tslib_1.__decorate([
...
tslib_1.__decorate([
    ...
    tslib_1.__metadata("design:paramtypes", []),
    tslib_1.__metadata("design:returntype", Promise)
], TestGitRepo.prototype, "findRefFiles", null);
tslib_1.__decorate([
    mocha_typescript_1.test,
    tslib_1.__metadata("design:type", Function),
    tslib_1.__metadata("design:paramtypes", []),
    tslib_1.__metadata("design:returntype", Promise)
], TestGitRepo.prototype, "readNonpackedRef", null);
tslib_1.__decorate([
    mocha_typescript_1.test,
    mocha_typescript_1.skip,
    tslib_1.__metadata("design:type", Function),
...
tslib_1.__decorate([
    ...
    tslib_1.__metadata("design:paramtypes", []),
    tslib_1.__metadata("design:returntype", Promise)
], TestGitRepo.prototype, "readObjRaw1", null);
tslib_1.__decorate([
    mocha_typescript_1.test,
    tslib_1.__metadata("design:type", Function),
    tslib_1.__metadata("design:paramtypes", []),
    tslib_1.__metadata("design:returntype", Promise)
], TestGitRepo.prototype, "readObjRaw2", null);
tslib_1.__decorate([
    mocha_typescript_1.test,
    tslib_1.__metadata("design:type", Function),
    tslib_1.__metadata("design:paramtypes", []),
    tslib_1.__metadata("design:returntype", Promise)
], TestGitRepo.prototype, "readCommitObj", null);
tslib_1.__decorate([
    mocha_typescript_1.test,
    tslib_1.__metadata("design:type", Function),
    tslib_1.__metadata("design:paramtypes", []),
    tslib_1.__metadata("design:returntype", Promise)
], TestGitRepo.prototype, "readAtagObj", null);
tslib_1.__decorate([
    mocha_typescript_1.test,
    tslib_1.__metadata("design:type", Function),
    tslib_1.__metadata("design:paramtypes", []),
    tslib_1.__metadata("design:returntype", Promise)
], TestGitRepo.prototype, "readAllObjects1", null);
tslib_1.__decorate([
    mocha_typescript_1.test,
    mocha_typescript_1.timeout(100e3),
    mocha_typescript_1.skip // this case requires a bare repo of TypeScript
    ,
    tslib_1.__metadata("design:type", Function),
    tslib_1.__metadata("design:paramtypes", []),
    tslib_1.__metadata("design:returntype", Promise)
], TestGitRepo.prototype, "readAllObjects2", null);
tslib_1.__decorate([
    mocha_typescript_1.test,
    tslib_1.__metadata("design:type", Function),
    tslib_1.__metadata("design:paramtypes", []),
    tslib_1.__metadata("design:returntype", Promise)
], TestGitRepo.prototype, "resolveRef", null);
